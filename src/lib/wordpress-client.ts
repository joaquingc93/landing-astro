...
const WORDPRESS_API_URL = process.env.WORDPRESS_API_URL || 'http://renovalinksite.local/wp-json';
const WORDPRESS_CUSTOM_API_URL = process.env.WORDPRESS_CUSTOM_API_URL || 'http://renovalinksite.local/wp-json/renovalink/v1';
...
class WordPressClient {
  ...
  async getAllPosts(): Promise<WPPost[]> {
    return this.fetchFromAPI('wp/v2/posts', WPPostSchema, { status: 'publish' });
  }
  ...
  async getAllServices(): Promise<Service[]> {
    console.log('🌐 Fetching services from:', `${this.baseUrl}/wp/v2/servicios`);
    try {
      const services = await this.fetchFromAPI('wp/v2/servicios', ServiceSchema, { status: 'publish' });
  ...
  async getAllTestimonials(): Promise<Testimonial[]> {
    return this.fetchFromAPI('wp/v2/testimonios', TestimonialSchema, { status: 'publish' });
  }
  ...
  async getMediaById(id: number): Promise<any | null> {
    ...
    try {
      const response = await this.fetchWithErrorHandling(`${this.baseUrl}/wp/v2/media/${id}`);
  ...
  async getCompanyInfo(): Promise<CompanyInfo | null> {
    ...
    try {
      const response = await this.fetchWithErrorHandling(`${this.customUrl}/company-info`);
      const data = await response.json();
  ...
  async getSiteConfig(): Promise<any> {
    ...
    try {
      const response = await this.fetchWithErrorHandling(`${this.customUrl}/site-config`);
      const data = await response.json();
  ...
  async getHeroContent(): Promise<any> {
    ...
    try {
      const response = await this.fetchWithErrorHandling(`${this.customUrl}/hero-content`);
      const data = await response.json();
      ...
      return {
        ...
        description: 'From pool renovations to structural engineering, we bring your vision to life with quality craftsmanship and certified expertise.',
  ...
  async healthCheck(): Promise<boolean> {
    try {
      const response = await this.fetchWithErrorHandling(`${this.baseUrl}/wp/v2/posts?per_page=1`);
      return response.ok;
    } catch {
      return false;
    }
    ...
    services: [
      {
        ...
        content: { rendered: 'Transform your pool with lighting, waterfalls, and modern features.' },
      ...
      },
      {
        ...
        content: { rendered: 'Interior and exterior concrete work, repairs, and decorative solutions.' },
      ...
      },
      {
        ...
        content: { rendered: 'Deep cleaning and recurring maintenance with eco-friendly products.' },
      ...
      },
      {
        ...
        content: { rendered: 'Certified engineering support and structural planning services.' },
    ...
    testimonials: [
      {
        ...
        content: { rendered: 'RenovaLink transformed our backyard completely. The attention to detail was exceptional.' },
